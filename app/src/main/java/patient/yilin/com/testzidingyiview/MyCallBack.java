...
public class MyCallBack extends ItemTouchHelper.Callback{
    ...
    private List<String> originImages ;  //图片未经过处理 传这个进来是为了使原图片的顺序与拖拽顺序保持一致
    private boolean up ;  //手指抬起标记为
    public MyCallBack(PostArticleImgAdapter postArticleImgAdapter, List<String> images,List<String> originImages){
    ...
    /**
     *  设置 item  是否处理拖拽事件 和滑动事件  以及拖拽和滑动操作的方向
     * @param recyclerView
     * @param viewHolder
     * @return
     */
    ...
    /**
     *   当用户 从item 原来位置拖动可以拖动到item 新位置的过程中的调用
     * @param recyclerView
     * @param viewHolder
     * @param target
     * @return
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, RecyclerView.ViewHolder target) {
        ...
        int toPosition = target.getAdapterPosition() ;  //得到target 的位置
    ...
    /**
     *  当用户与item 的交互结束 并且item也完成了动画时调用
     * @param recyclerView
     * @param viewHolder
     */
    @Override
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if(dY>=(recyclerView.getHeight()-viewHolder.itemView.getBottom()  //item  底部距离recyclerView 的顶部高度
            ...
            if(View.INVISIBLE == viewHolder.itemView.getVisibility()){  //如果viewholder  不可见 则表示用户放手 重置删除区域状态
                dragListener.dragState(false);
            }
            dragListener.deleteState(false);
        }
    ...
    /**
     * s设置手指离开后viewholder 的动画时间  在用户手指离开后调用
     * @param recyclerView
     * @param animationType
     * @param animateDx
     * @param animateDy
